...
- Event stream은 N개의 event packet의 시퀀스로 분해됨 → $\mathcal E$
    - $\mathcal {E} = \{\mathcal {E}_{t_i}\}_{i=1}^{N}$ | $\mathcal E_{t_i}$ : $t_{i-1}$에서 $t_i$사이의 event를 모은 것
...
- 두 flow 모두 인간 움직임에 기반 → motion estimation 문제를 정규화 → *안정적으로 학습, 일반화 성능 향상*
    - Optical flow → **Image-based flow** ( $\mathbf {F}^{\text {img}}_{t_i}$ )
        - $\mathbf {F}^{\text {img}}_{t_i} = \mathrm{BilinearSample}(O_{t_i}, \pi (\mathbf {\hat v}_{t_{i-1}})$
...
- $\mathcal {L}_{\text {flow}} = \sum\limits _{t_i}\sum\limits_{v} \frac {\langle \mathbf {F}^{\text {shape}}_{t_{i}, v}, \mathbf {F}^{\text {img}}_{t_{i}, v}\rangle }{\|\mathbf {F}^{\text {shape}}_{t_{i}, v}\|_2 \cdot \|\mathbf {F}^{\text {img}}_{t_{i}, v}\|_2}$
